...
#### Start solution ####
#   Answers #
#Ans.of Question no.1.1 and 1.2
x <- c("A","L","E","E","N","A", "M","U","S","H","T","A","Q")
x
# Ans. 1.3
x <- c(100:120)
x
...
dim(m) <- c(3,3)
m
# I couldn't find the code for making sequence of all even numbers.
...
```{r }
x <- factor(c("Employed", "unemployed", "self-employed"))
x
# Ans. 2
table(x)
unclass(x)
...
```{r }
##Data Frame Object:
# Title: bank_customers
bank_customers <- data.frame (1:5) 
                Value <- c(NA, 10000, 100, 2000, NA )
                customer_names <- c("Alicia", "Nancy", "Fernando", "Louis","Diane")
colnames <-`c("CustomerID", "hasAccount", "totalBalance")
df <- data.frame(colnames, customer_Names, Value)
